    ...
    return (
        <div className="profile-seller mt-4 " >
            <p className="alert alert-warning mr-5 mb-3 pl-2" style={{ fontSize: '12px', marginTop: '0px' }}>
                <AiOutlineWarning /> You must complete your profile before you add your product and you can update your restaurant profile here</p>
...
{/* {
                                editResto.taxservice === "yes" ?
                                    <select onChange={handleSelect} name="taxservice" style={{ fontSize: "12px" }} className="form-control" size="sm">
                                        <option value="yes" style={{ fontSize: "12px" }} className="form-control" selected>Yes</option>
                                        <option value="no" style={{ fontSize: "12px" }} className="form-control">No</option>
                                    </select>
                                    : editResto.taxservice === "no" ?
                                        <select onChange={handleSelect} name="taxservice" style={{ fontSize: "12px" }} className="form-control" size="sm">
                                            <option value="no" style={{ fontSize: "12px" }} className="form-control" selected>No</option>
                                            <option value="yes" style={{ fontSize: "12px" }} className="form-control" >Yes</option>
                                        </select>
                                        :
                                        (<select onChange={handleSelect} name="taxservice" style={{ fontSize: "12px" }} className="form-control" size="sm">
...
/*
        ...
        Axios.get(`${APIURL}produk/scheduleweekday/${id}`)
            .then(res3 => {
                setselectweekday(res3.data)
            })
        Axios.get(`${APIURL}produk/scheduleweekend/${id}`)
            .then(res4 => {
                setselectweekend(res4.data)
            })
        Axios.get(`${APIURL}produk/scheduleholiday/${id}`)
            .then(res4 => {
                setselectholiday(res4.data)
            })
*/
...
/*
    ...
    //     }
    //     return listSchedule.map((val, i) => {
    //         if (selectweekday[0].schedule === val.schedule) return <option key={i} value={val.schedule} selected>{val.schedule}</option>
    //         return < option key={i} value={val.schedule} > {val.schedule}</option >
    //     })
    // }
    ...
    // --- weekend ---
    // const scheduleweekend = () => {
    //     if (selectweekend.length === 0) {
    //         return listSchedule.map((val, i) => {
    //             return <option key={i} value={val.schedule}>{val.schedule}</option>
    //         })
    //     }
    //     return listSchedule.map((val, i) => {
    //         if (selectweekend[0].schedule === val.schedule) return <option key={i} value={val.schedule} selected>{val.schedule}</option>
    //         return < option key={i} value={val.schedule} > {val.schedule}</option >
    //     })
    // }
    ...
    //     }
    //     return listSchedule.map((val, i) => {
    //         // console.log('holiday', selectholiday[0].schedule)

    //         if (selectholiday[0].schedule === val.schedule) return <option key={i} value={val.schedule} selected>{val.schedule}</option>
    //         return < option key={i} value={val.schedule} > {val.schedule}</option >
    //     })
    // }
...
<div className="text-center">
                ...
                <div className="mx-auto w-50">
                    ...
                    <div className="d-flex">
                        <Label className="mr-5">Others: </Label>
                        <FormGroup className="w-50 mr-2">
                    ...
                    <div className="jadwal">
                        <Label className="mr-5">Operational Hours </Label>
                        <FormGroup className="d-flex ml-3">
                            <Label sm={2}>Sunday</Label>
                            <Col className="d-flex">
                                <Input type="time" className="form-control w-25 mr-2" size="sm" /> -
                                <Input type="time" className="form-control w-25 ml-2" size="sm" />
                            </Col>
                        </FormGroup>
